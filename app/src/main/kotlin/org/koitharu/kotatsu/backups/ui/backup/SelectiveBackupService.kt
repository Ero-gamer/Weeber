...
/**
 * Service for creating selective backups with user-chosen sections.
 * Unlike [BackupService] which backs up everything, this allows backing up specific sections only.
 */
@AndroidEntryPoint
@SuppressLint("InlinedApi")
class SelectiveBackupService : BaseBackupRestoreService() {
	...
	override suspend fun IntentJobContext.processIntent(intent: Intent) {
		...
		val sectionNames = intent.getStringArrayExtra(EXTRA_SECTIONS) ?: throw IllegalArgumentException("No sections specified")
		val sections = sectionNames.mapNotNull { name ->
			BackupSection.entries.find { it.name == name }
		}.toSet()
		
		if (sections.isEmpty()) {
			throw IllegalArgumentException("No valid sections specified")
		}
		...
		powerManager.withPartialWakeLock(TAG) {
			...
			try {
				ZipOutputStream(contentResolver.openOutputStream(destination)).use { output ->
					repository.createSelectiveBackup(output, sections, progress)
				}
			} catch (e: Throwable) {
				try {
			...
			withContext(Dispatchers.Main) {
				Toast.makeText(this@SelectiveBackupService, R.string.backup_saved, Toast.LENGTH_SHORT).show()
			}
		}
	}
	...
	companion object {
		...
		private const val TAG = "SELECTIVE_BACKUP"
		private const val FOREGROUND_NOTIFICATION_ID = 34
		private const val EXTRA_SECTIONS = "sections"

		@CheckResult
		fun start(context: Context, uri: Uri, sections: Set<BackupSection>): Boolean = try {
			val intent = Intent(context, SelectiveBackupService::class.java)
			intent.putExtra(AppRouter.KEY_DATA, uri.toString())
			intent.putExtra(EXTRA_SECTIONS, sections.map { it.name }.toTypedArray())
