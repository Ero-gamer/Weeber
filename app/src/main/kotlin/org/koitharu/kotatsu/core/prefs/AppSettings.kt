...
class AppSettings @Inject constructor(@ApplicationContext context: Context) {
	...
	var updateCheckIntervalHours: Int
		get() = prefs.getInt(KEY_UPDATE_CHECK_INTERVAL, 24)
		set(value) = prefs.edit { putInt(KEY_UPDATE_CHECK_INTERVAL, value) }

	var isUpdateCheckWifiOnly: Boolean
		get() = prefs.getBoolean(KEY_UPDATE_CHECK_WIFI_ONLY, false)
		set(value) = prefs.edit { putBoolean(KEY_UPDATE_CHECK_WIFI_ONLY, value) }

	fun isVersionSkipped(versionName: String): Boolean =
		prefs.getBoolean(KEY_SKIPPED_VERSION_PREFIX + versionName, false)

	fun skipVersion(versionName: String) =
		prefs.edit { putBoolean(KEY_SKIPPED_VERSION_PREFIX + versionName, true) }
	...
	var defaultDownscaleMode: DownscaleMode
		...
		set(value) = prefs.edit { putString(KEY_READER_DOWNSCALE_MODE, value.name) }
	...
	companion object {
		...
		const val KEY_LAST_UPDATE_CHECK = "last_update_check_time"
		const val KEY_UPDATE_CHECK_INTERVAL = "update_check_interval_hours"
		const val KEY_UPDATE_CHECK_WIFI_ONLY = "update_check_wifi_only"
		const val KEY_SKIPPED_VERSION_PREFIX = "skipped_version_"
		const val KEY_LAST_VACUUM = "last_vacuum_time"
