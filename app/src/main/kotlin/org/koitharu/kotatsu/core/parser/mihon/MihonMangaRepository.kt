 ...
 */
internal fun List<SChapter>.toKotatsuChapterOrder(): List<OrderedMihonChapter> = asReversed()
...
class MihonMangaRepository(
	...
	override suspend fun getList(offset: Int, order: SortOrder?, filter: MangaListFilter?): List<Manga> = withContext(Dispatchers.IO) {
		runCatching {
			...
			mangas.mangas.map { it.toKotatsuManga() }
	...
	override suspend fun getDetailsImpl(manga: Manga): Manga = withContext(Dispatchers.IO) {
		...
		val chapters = loadChapters(seed, details).toKotatsuChapterOrder().map { (chapter, number) ->
			chapter.toKotatsuChapter(number = number)
		}
		details.toKotatsuManga(chapters = chapters).copy(id = manga.id)
	}
	...
	private fun SManga.toKotatsuManga(chapters: List<MangaChapter>? = null): Manga {
		val rawUrl = safeUrl()
	...
	private fun SChapter.toKotatsuChapter(number: Float): MangaChapter {
		val rawUrl = safeUrl()
		return MangaChapter(
	...
	private fun Page.toKotatsuPage(index: Int): MangaPage {
