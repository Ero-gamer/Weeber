...
internal object CrashReportFormatter {
	...
	private const val FALLBACK_ISSUE_URL = "https://github.com/Ero-gamer/Weeber/issues/new"
	...
	private fun buildDegraded(context: Context, throwable: Throwable): CrashReportData {
		...
		return CrashReportData(
			title = title,
			body = "Weeber crashed but the full crash report could not be collected.",
			issueUrl = issueUrl,
			isDegraded = true,
		)
	}
	...
	private fun buildIssueUrl(
		...
		val baseUrl = context.getString(R.string.url_error_report)
			.trim()
			.removeSuffix("/choose")
			.ifEmpty { "https://github.com/Ero-gamer/Weeber/issues/new" }
		return Uri.parse(baseUrl).buildUpon()
			.appendQueryParameter("template", ISSUE_TEMPLATE)
			.appendQueryParameter("title", title)
