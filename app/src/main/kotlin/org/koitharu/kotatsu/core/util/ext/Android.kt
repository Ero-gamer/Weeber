...
import org.koitharu.kotatsu.core.network.webview.WebViewPerformanceConfigurator
...
fun WebView.configureForParser(userAgentOverride: String?) = with(settings) {
	// Apply hardware layer, high render priority, disable Safe Browsing and mixed content block
	WebViewPerformanceConfigurator.applyPerformanceSettings(this@configureForParser)
	...
	if (userAgentOverride != null) {
		// Strip the "; wv)" embedded-WebView token from the UA string.
		// This marker raises bot scores on Cloudflare and similar WAFs.
		userAgentString = WebViewPerformanceConfigurator.stripWebViewMarker(userAgentOverride)
	} else {
		// Also strip the token from the default UA when no override is provided
		userAgentString = WebViewPerformanceConfigurator.stripWebViewMarker(userAgentString)
	}
